...
Run these commands in a scratch directory outside your Terraform checkout.
...
uv pip install --python .venv/bin/python --index-url https://test.pypi.org/simple --no-deps iaclineage==0.1.0b1
...
uv pip install --python .venv\Scripts\python.exe --index-url https://test.pypi.org/simple --no-deps iaclineage==0.1.0b1
...
Check the version is `0.1.0b1`, open the report locally, find a known declaration,
